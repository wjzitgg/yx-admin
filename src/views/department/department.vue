<template>
  <div class="app-container">
    ...
    <div class="table">
      ...
      <!-- 添加编辑弹出框 -->
      <el-dialog :title="form.id?'部门修改':'新增部门'" :visible.sync="editVisible" width="40%">
    ...
    methods: {
      ...
      addAssets(row) {
        ...
        saveEdit(form) {
            ...
            if(!form.id){
                // 添加
                this.$post('department/department_add',{
                    name:this.form.name,
                ...
                // 编辑修改
                this.$post('department/department_upd',{
                    id:this.form.id,
                    name:this.form.name,
                }).then((res) => {
                    if(res.code == "success"){
      ...
      // 确定删除
      deleteRow(id){
          this.$post('department/department_del',{
              id:id
          }).then((res) => {
              if(res.code == "success"){
                  this.getData();
